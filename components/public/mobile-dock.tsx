...
export function MobileDock() {
  ...
  function openSearch() {
    ...
    window.setTimeout(() => {
      document.getElementById("ara")?.scrollIntoView({ behavior: "smooth", block: "center" });
      ...
      <ul className="mx-auto grid max-w-lg grid-cols-4 gap-1 px-2 pt-1.5">
        {items.map((item) => {
          ...
          const iconWrap = (
            ...
            >
              <Icon
                className={cn("h-4 w-4", item.href === "/favoriler" && count > 0 && active && "fill-current")}
              />
            </span>
          );
          ...
          return (
            <li key={`${item.href}-${item.label}`}>
              {item.focusSearch ? (
                <button type="button" onClick={openSearch} className={cn(className, "w-full")}>
                  {iconWrap}
                  {item.label}
                </button>
              ) : (
